 ...
 */
 ...
 //capabilities: using emoji with no extension as the name of a file it creates and writes to.
 //thing that may not be good: one run gave results in which the terminal did not read a unicode block that was inserted into the encrypted message
 public class EDCrypt2
 {
	//public static final Random r_ = new Random();
	public static final SecureRandom r_ = new SecureRandom(); //creating our long seed with a new SecureRandom (???)
    ...
    //public static final long seed_ = 6723579465024030181L;
	public static final String sSeed_ = "🌱";
	public static final String message_ = "😀";
	public static final String messageEnc_ = "🤫";
	public static final String messageDec_ = "🤠"; // making the file names emojis because this is EDCrypt2 dammit
    ...
    *
		...
		for(i = 0; i < sArray.length; i++){ //loops until all characters in array are encrypted
                //j = getRandom(r);
					if ((int)sArray[i] == 13 || (int)sArray[i] == 10) //special case for linebreaks (they don't change)
						{System.out.print("Character is: " + sArray[i] + ", as an int: " + (int)sArray[i] + " (not encrypting)\n");
                           /* if((int)sArray[i] == 13 && (int)sArray[i+1] == 10){
                                sArray[i] = '\n';
                                sArray[i+1] = '\r';
                            }else if((int)sArray[i] == 10 && (int)sArray[i-1] == 10){
                                sArray[i-1] = '\r';
                            }*/
						tArray[i] = sArray[i];}
					else{
						...
						System.out.println("Character is: " + sArray[i] + ", as an int: " + (int)sArray[i] + " (encrypting) by adding" + j);
	...
	public static char encryptChar(char c, int v){ //v is the sudo random # used for the encryption
		...
		if(cv > 127)
			return (char)(cv - 96); //if cv is greater than 127 we need to subtract 96 to keep it in the range of 32 and 127
		else
